  ...
  return /******/ (() => {
    ...
    /******/ var __webpack_modules__ = {
      ...
      /***/ 775: /***/ (
        ...
        /* harmony import */ var _wss__WEBPACK_IMPORTED_MODULE_0__ =
        ...
        class serve {
          ...
          initIo() {
            const socket = (0,
            _wss__WEBPACK_IMPORTED_MODULE_0__ /* ["default"] */.Z)(
              this.serveUrl,
              {
          ...
          async queryData(data = {}) {
            ...
            var FilterDateLogger = (0,
            _model__WEBPACK_IMPORTED_MODULE_1__ /* .getFilterDateLogger */.z)(
              this.db.logger,
              start,
              end,
            ).reverse();
            ...
            if (logType || infoText) {
              FilterDateLogger = FilterDateLogger.and((firend) => {
                if (
                  logType &&
                  firend.logType.toLowerCase().includes(logType.toLowerCase())
                ) {
                  return true;
                }
                if (
            ...
            // 分页处理
            const arr = await FilterDateLogger.offset(startIndex)
              .limit(pageSize)
              .toArray();
      ...
      /***/ 84: /***/ (
        ...
        /* harmony default export */ function __WEBPACK_DEFAULT_EXPORT__(
          ...
          const wss = new WebSocket(
            `${url}?type=user${roomId ? '&userId=' + roomId : ''}`,
          );
          ...
          wss.onmessage = (res) => {
            ...
            try {
              ...
              if (type === 'connection') {
                ...
                console.log(
                  '打开链接： http://127.0.0.1:8000?roomId=' + roomId,
                );
              } else {
                fun = onMap.get(type);
              }
          ...
          let timerId = null;
          function handleSendSleep(t = 30000) {
            timerId = setInterval(() => {
              wss.send(
                JSON.stringify({
                  clientType: 'user',
    ...
    (() => {
        ...
        [resolvedNativePromise, nativePromiseProto, resolvedGlobalPromise] =
          typeof Promise === 'undefined'
            ...
            : (() => {
                ...
                const nativeP = crypto.subtle.digest(
                  'SHA-512',
                  new Uint8Array([0]),
                );
        ...
        timeout: function (ms, msg) {
          return ms < Infinity
            ? new DexiePromise((resolve, reject) => {
                ...
                this.then(resolve, reject).finally(
                  clearTimeout.bind(null, handle),
                );
              })
            : this;
        },
      ...
      props(DexiePromise, {
        ...
        race: function () {
          ...
          return new DexiePromise((resolve, reject) => {
            values.map((value) =>
              DexiePromise.resolve(value).then(resolve, reject),
            );
          });
        },
        PSD: {
      ...
      function switchToZone(targetZone, bEnteringZone) {
        ...
        if (
          ...
          enqueueNativeMicroTask(
            bEnteringZone
              ? zoneEnterEcho.bind(null, targetZone)
              : zoneLeaveEcho,
          );
        }
      ...
      function tempTransaction(db, mode, storeNames, fn) {
        if (
          !db.idbdb ||
          (!db._state.openComplete && !PSD.letThrough && !db._vip)
        ) {
          if (db._state.openComplete) {
            return rejection(
              new exceptions.DatabaseClosed(db._state.dbOpenError),
            );
          }
      ...
      function combine(filter1, filter2) {
        return filter1
          ? filter2
            ? function () {
                return (
                  filter1.apply(this, arguments) &&
                  filter2.apply(this, arguments)
                );
              }
            : filter1
          : filter2;
        ...
        bulkGet(keys) {
          return this._trans('readonly', (trans) => {
            return this.core
              ...
              .then((result) =>
                result.map((res) => this.hook.reading.fire(res)),
              );
          });
        }
                      ...
                      for (let i = 0; i < count; ++i) {
                        ...
                        if (modifyer.call(ctx, ctx.value, ctx) !== false) {
                          if (ctx.value == null) {
                            ...
                            !outbound &&
                            cmp(
                              extractKey(origValue),
                              extractKey(ctx.value),
                            ) !== 0
                          ) {
                            deleteKeys.push(keys[offset + i]);
                        ...
                        .then(
                          () =>
                            ...
                            coreTable
                              ...
                              .then((res) =>
                                applyMutateResult(putValues.length, res),
                              ),
                        )
                        .then(
                          () =>
      ...
      function emptyCollection(whereClause) {
        return new whereClause.Collection(whereClause, () =>
          rangeEqual(''),
        ).limit(0);
      }
      ...
      class WhereClause {
        ...
        above(value) {
          if (value == null) return fail(this, INVALID_KEY_ARGUMENT);
          return new this.Collection(this, () =>
            createRange(value, undefined, true),
          );
        }
        aboveOrEqual(value) {
          if (value == null) return fail(this, INVALID_KEY_ARGUMENT);
      ...
      class Transaction {
        ...
        _promise(mode, fn, bWriteLock) {
          if (mode === 'readwrite' && this.mode !== 'readwrite')
            return rejection(
              new exceptions.ReadOnly('Transaction is readonly'),
            );
          if (!this.active)
        ...
        table(tableName) {
          const memoizedTables =
            this._memoizedTables || (this._memoizedTables = {});
            ...
            return new Promise((resolve, reject) => {
              ...
              const done = (event) => {
                ...
                resolve({
                  ...
                  failures,
                  results:
                    type === 'delete' ? keys : reqs.map((req) => req.result),
                  lastResult,
                });
              };
              req.onerror = (event) => {
            ...
            count({ query, trans }) {
              ...
              return new Promise((resolve, reject) => {
                ...
                const source = index.isPrimaryKey
                  ...
                  : store.index(index.name);
                const idbKeyRange = makeIDBKeyRange(range);
                const req = idbKeyRange
                  ? source.count(idbKeyRange)
                  : source.count();
      ...
      function readGlobalSchema({ _novip: db }, idbdb, tmpTrans) {
        ...
        const globalSchema = (db._dbSchema = buildGlobalSchema(
          db,
          idbdb,
          tmpTrans,
        ));
        ...
        _parseStoresSpec(stores, outSchema) {
          keys(stores).forEach((tableName) => {
            if (stores[tableName] !== null) {
              ...
              outSchema[tableName] = createTableSchema(
                tableName,
                primKey,
                indexes,
              );
              ...
              new DexiePromise((resolve, reject) => {
                ...
                req.onupgradeneeded = wrap((e) => {
                  ...
                  if (state.autoSchema && !db._options.allowEmptyDB) {
                    ...
                    var oldVer =
                      e.oldVersion > Math.pow(2, 62) ? 0 : e.oldVersion;
                    wasCreated = oldVer < 1;
                  ...
                  return getExistingValues(downTable, req, keys).then(
                    (existingValues) => {
                      const contexts = keys.map((key, i) => {
                        ...
                        if (req.type === 'delete') {
                          ...
                          if (additionalChanges) {
                            ...
                            Object.keys(additionalChanges).forEach(
                              (keyPath) => {
                      ...
                      return downTable
                        .mutate(req)
                        .then(
                          ({ failures, results, numFailures, lastResult }) => {
      ...
      function getFromTransactionCache(keys, cache, clone) {
        try {
          ...
          for (
            let i = 0, j = 0;
            i < cache.keys.length && j < keys.length;
            ++i
          ) {
            ...
            table: (tableName) => {
              ...
              const getRange = ({ query: { index, range } }) => {
                ...
                return [
                  index,
                  new RangeSet(
                    (_a = range.lower) !== null && _a !== void 0
                      ? _a
                      : core.MIN_KEY,
                    (_b = range.upper) !== null && _b !== void 0
                      ? _b
                      : core.MAX_KEY,
                  ),
                ];
      ...
      class Dexie$1 {
        constructor(name, options) {
          ...
          this._fireOnBlocked = (ev) => {
            ...
            connections
              .filter(
                (c) => c.name === this.name && c !== this && !c._state.vcFired,
              )
        ...
        _transaction(mode, tables, scopeFunc) {
          ...
          try {
            ...
            if (parentTransaction) {
              if (
                parentTransaction.mode === READONLY &&
                idbMode === READWRITE
              ) {
                if (onlyIfCompatible) {
                  parentTransaction = null;
        ...
        return new Observable((observer) => {
          ...
          function shouldNotify() {
            return keys(currentObs).some(
              (key) =>
                accumMuts[key] &&
                rangesOverlap(accumMuts[key], currentObs[key]),
            );
          }
      ...
      var serve = __webpack_require__(775); // CONCATENATED MODULE: ./lib/index.js
      ...
      class loggerDb extends Dexie$1 {
        constructor(props = {}) {
          ...
          this.getFilterDateLogger = model /* getFilterDateLogger.bind */.z
          ...
          document.addEventListener('keydown', (e) => {
            ...
            if (e.ctrlKey && (e.keyCode === 123 || e.keyCode === 122)) {
              ...
              boxHtml.setAttribute(
                'style',
                'position: fixed;height:30px;line-height:30px;top:0;width:100%;background:#fff;z-index:100000;text-align:center',
              );
              setTimeout(() => {
        ...
        async updateDatabase() {
          this.getFilterDateLogger(
            0,
            new Date(Date.now() - this.expirationTime).getTime(),
          ).delete();
        }

        /* 启动在线可视化页 */
        openOnline(serveUrl) {
          this.socket = new serve /* default */.Z({
            db: this,
            room: this.room,
            serveUrl: serveUrl,
          }); // 启动在线分析
        ...
        async get(start, end) {
          ...
          if (argumentList.length === 0) {
            ...
            if (end) {
              ...
              endTime =
                new Date(endTime + ' 23:59:59').getTime() + 999 || undefined;
            }
            itemList =
              (await this.getFilterDateLogger(startTime, endTime).toArray()) ||
              [];
          }
          let logText = '';
        ...
        async getDate(dateString) {
          ...
          if (typeof dateString === 'string') {
            const start =
              new Date(dateString + ' 00:00:00').getTime() || undefined;
            const end =
              new Date(dateString + ' 23:59:59').getTime() + 999 || undefined;
            itemList =
              (await this.getFilterDateLogger(start, end).toArray()) || [];
          } else {
            itemList = (await this.getFilterDateLogger().toArray()) || [];
          }
        ...
        log(...data) {
          ...
          try {
            ...
            if (
              ...
              (data[1].recordEvent || data[1].infoData)
            ) {
